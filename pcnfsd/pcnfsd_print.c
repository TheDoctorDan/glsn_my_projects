/* RE_SID: @(%)/net/shipyard/files2/builds/capybara/unix/pcnfsvr/pcnfsd/SCCS/s.pcnfsd_print.c 1.2 94/09/21 23:58:48 SMI */

/*
**=====================================================================
** Copyright (c) 1986-1994 by Sun Microsystems, Inc.
**	@(#)pcnfsd_print.c	1.2	21 Sep 1994
**=====================================================================
*/
#include "common.h"
/*
	   ...
	   ** Should never happen.
		...
		*/
...
#ifdef GLEESON_STYLE_PRINT
			sprintf(cmdbuf, "/usr/gleeson/bin/prt -b -nf -pt lp%s -fm 1 -dt PRINTER %s",
				pr, new_pathname);
#endif GLEESON_STYLE_PRINT
		...
		if ((fd = su_popen(user, xcmd, MAXTIME_FOR_PRINT)) == NULL) {
			...
			msg_out("pcnfsd_print: su_popen failed");
			sprintf(mess,"command:%s",xcmd);
			msg_out(mess);
			return(PS_RES_FAIL);
		}

#ifdef GLEESON_STYLE_PRINT
		strcpy(req_id,"123"); 	/* asume seccess */
...
#endif
#ifdef GLEESON_STYLE_MONITOR
	char *mon_path = LPDEV_FILENAME;
#endif
...
int build_pr_list()
{
	...
	flpdev = open(LPDEV_FILENAME, 0);
	if(flpdev<0){
		sprintf(mess, "rpc.pcnfsd: unable to open() %s file", LPDEV_FILENAME);
		msg_out(mess);
		return(1);
	}

	strcpy(lpdev,"");
	while(isam_next_read(flpdev, lpdev, &lpdev_rec) >0l){

		curr = (struct pr_list_item *)
			grab(sizeof (struct pr_list_item));

		curr->pn = my_strdup(lpdev);
		curr->device = NULL;
		curr->remhost = NULL;
		curr->cm = my_strdup(lpdev_rec.desc);
		curr->pr_next = NULL;
		curr->device = my_strdup(lpdev);
		curr->remhost = my_strdup("");
		if(last == NULL)
			printers = curr;
		else
...
#ifdef GLEESON_STYLE_QUEUE
/*
 * Gleeson style queue listing
 */

pirstat build_pr_queue(pn, user, just_mine, p_qlen, p_qshown)
printername     pn;
username        user;
int            just_mine;
int            *p_qlen;
int            *p_qshown;
{
	...
	*p_qshown = 0;
	...
	flpspool = open(LPSPOOL_FILENAME, 0);
	if(flpspool <0){
		sprintf(mess, "rpc.pcnfsd: unable to open() %s file", LPSPOOL_FILENAME);
		msg_out(mess);
		return(PI_RES_FAIL);
	}
	...
	*lpspool=0;
	while(isam_next_read(flpspool, lpspool, &lpspool_rec) >0l){
		if(strcmp(lpspool_rec.lpdev, pn) !=0)
			continue;

		passwd_ptr = getpwuid(lpspool_rec.uid);
		if (passwd_ptr != NULL) {
			owner=passwd_ptr->pw_name;
		} else {
			continue;
		}
		...
		for(lp_item=0;lp_item<2; lp_item++){
			if (*lpspool_rec.spool_filename[lp_item] == 0)
				continue;

			sprintf(totsize,"%7d", lpspool_rec.byte_count[lp_item]);

			files=lpspool_rec.orig_filename[lp_item];

			sprintf(rank,"%3d", lpspool_rec.priority);

			sprintf(job,"%s-%d", lpspool, lp_item);
			...
			*p_qshown += 1;
			...
			curr->position = *p_qshown;
			curr->id = my_strdup(job);
			curr->size = my_strdup(totsize);

			if(lpspool_rec.hold){
				curr->status = my_strdup("Hold");
			}else{
				if(lpspool_rec.copies_requested[lp_item]==0){
					curr->status = my_strdup("Purged");
				}else{
					if (isam_find_read(flpdev, lpspool_rec.lpdev, &lpdev_rec) <= 0l)  {
						memset(&lpdev_rec, 0, sizeof lpdev_rec);
					}
					if (strcmp(lpspool, lpdev_rec.printing_lpspool) == 0 && lp_item == lpdev_rec.printing_lp_item)
						curr->status = my_strdup("Print");
					else
						curr->status = my_strdup("Wait");
				}
			}
...
#ifdef GLEESON_STYLE_STATUS

pirstat get_pr_status(pn, avail, printing, qlen, needs_operator, status)
printername   pn;
bool_t       *avail;
bool_t       *printing;
int          *qlen;
...
char         *status;
{
	...
	*status = '\0';
	...
	strncpyt(lpdev,pn, LPDEV_KEY_LEN);
	...
	*avail = TRUE;
	*printing = (lpdev_rec.stop_printing==0);

	if(lpdev_rec.stop_printing==0){
		if(strcmp(lpdev_rec.printing_lpspool,"")!=0){
			strcpy(status, "Printing");
			time(&now_time);
			if((now_time - lpdev_rec.last_print_time) >600){
				*needs_operator = TRUE;
...
/*
** If it failed, try one more time with the job id appended
** to the printer name.
*/
	if(stat != PC_RES_OK) {
...
char *id;
{
	...
	strncpyt(lpspool, id, LPSPOOL_KEY_LEN);
	if((lpspool_recl = isam_find_lock_read(flpspool, lpspool, &lpspool_rec)) <=0l){
		r_unlock(flpspool, lpspool_recl, sizeof lpspool_rec);
		close(flpspool);
		return(PC_RES_NO_SUCH_JOB);
	}

	if (sscanf(&id[LPSPOOL_KEY_LEN+1], "%d", &lp_item) != 1) {
		r_unlock(flpspool, lpspool_recl, sizeof lpspool_rec);
		close(flpspool);
		return(PC_RES_NO_SUCH_JOB);
	}
	...
	lpspool_rec.copies_requested[lp_item]=0;
	if(isam_insert_write_unlock(flpspool, lpspool, &lpspool_rec) <=0l){
		sprintf(mess, "rpc.pcnfsd: unable to write() %s file", LPSPOOL_FILENAME);
		msg_out(mess);
		r_unlock(flpspool, lpspool_recl, sizeof lpspool_rec);
		close(flpspool);
		return(PC_RES_FAIL);
...
#ifdef GLEESON_STYLE_PRINT
char default_cmd[] = "/usr/gleeson/bin/prt $FILE";
#endif GLEESON_STYLE_PRINT
