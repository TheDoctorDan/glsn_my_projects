/* RE_SID: @(%)/net/shipyard/files2/builds/capybara/unix/pcnfsvr/pcnfsd/SCCS/s.pcnfsd_misc.c 1.1 94/05/15 20:01:18 SMI */

/*
**=====================================================================
** Copyright (c) 1986-1994 by Sun Microsystems, Inc.
**	@(#)pcnfsd_misc.c	1.1	15 May 1994
**=====================================================================
*/
#include "common.h"
/*
	...
	int		maxtime;
{
	...
	if ((pid = fork()) == 0) {
		...
		msg_out("rpc.pcnfsd: execl failed");
		_exit(255);
	}
	if (pid == -1) {
	...
	if(pipe_handle==NULL){
		msg_out("rpc.pcnfsd: pip_handel fdopen failed");
	}
	return (pipe_handle);
}
