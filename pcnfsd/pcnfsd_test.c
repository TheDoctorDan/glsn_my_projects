/* RE_SID: @(%)/net/shipyard/files2/builds/capybara/unix/pcnfsvr/pcnfsd/SCCS/s.pcnfsd_test.c 1.1 94/05/15 20:01:37 SMI */
...
FILE *fp;
	...
	if(rp->stat == PS_RES_OK)
		printf("stat = PS_RES_OK\n");
	if(rp->stat == PS_RES_ALREADY)
		printf("stat = PS_RES_ALREADY\n");
	if(rp->stat == PS_RES_NULL)
		printf("stat = PS_RES_NULL\n");
	if(rp->stat == PS_RES_NO_FILE)
		printf("stat = PS_RES_NO_FILE\n");
	if(rp->stat == PS_RES_FAIL)
		printf("stat = PS_RES_FAIL\n");
