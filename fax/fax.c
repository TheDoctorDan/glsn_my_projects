...
send_fdt:
						...
						/* data transmission */
						/* option _FDT[=DF,VR,WD,LN] */
						/* DF=data compression format, VR=vertical resolution */
						/* WD=page width,  LN=page length */
						send_to("AT+FDT\r");
			...
			if (ioctl(fileno(fax_in), TCSETA, &tty_setting)) {
				...
				goto bad_end;
						...
						/* allow responses in from modem. i.e. RING, +FCON, CONNECT */
						send_to("ATQ0\r");
						switch (wait_for("OK\r\n", 5, 0)) {
						case GOT_IT:
							break;
						...
						/* capability to receive. Can receive fax. i.e. not out of paper */
						send_to("AT+FCR=1\r");
						switch (wait_for("OK\r\n", 5, 0)) {
						case GOT_IT:
							break;
						case GOT_EOF:
							goto bad_end;
						case GOT_TIMEOUT:
							fprintf(stderr, "fax modem not responding to AT+FCR=1 with OK\n");
							goto bad_end;
						}

						/* auto answer modem parameter */
						/* 0 = answer for fclass setting only */
						/* 1 = answer for either class=0 data, or class=2 fax */
						send_to("AT+FAA=0\r");
						switch (wait_for("OK\r\n", 5, 0)) {
						case GOT_IT:
							break;
						case GOT_EOF:
							goto bad_end;
						case GOT_TIMEOUT:
							fprintf(stderr, "fax modem not responding to AT+FAA=0 with OK\n");
							goto bad_end;
						}
...
wait_for_dcs_dtc:
						...
						got_fcfr = 0;
send_fdr:
						/* Begin or Continue to receive phase C data comand */
						send_to("AT+FDR\r");
...
wait_for_del_etx:
						switch (wait_for_many(600, 1, "\020\020", "\020\003", "+FHNG", NULL)) {
						case GOT_IT:
							/* del del -> del */
							putc("\020", file_out);
							sent_control_s=1;
						...
						switch (wait_for_many(40, 0, "+FPTS", "+FHNG", NULL)) {
						case GOT_IT:
							/* +FPTS */
