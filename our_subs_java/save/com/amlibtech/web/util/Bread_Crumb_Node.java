...
public class Bread_Crumb_Node {
    ...
    public void dispatch(HttpServletResponse response) throws java.io.IOException {
	Nav_Base.forward(response, this.getDestination_URL());
    }

    public void reload_Dispatch(HttpServletResponse response) throws java.io.IOException {
	if(this.getReload_URL()==null || this.getReload_URL().equals("")){
		Nav_Base.forward(response, this.getDestination_URL());
	}else{
		Nav_Base.forward(response, this.getReload_URL());
	}
    }
