...
function menuBarItemOver(e) { // Private method
    ...
    if (this.parent.menuBarObj.activated) {
        if (!this.clicked) {
            menuBarObj = this.parent.menuBarObj;
            for (var i = 0, len = menuBarObj.childNodes.length; i < len; i++) {
                if (menuBarObj.childNodes[i].enabled && menuBarObj.childNodes[i].clicked) {
                    menuBarObj.childNodes[i].className = menuBarObj.childNodes[i].itemClassName;
                    if (menuBarObj.childNodes[i].iconObj) {
        ...
        menuBarObj = this.parent.menuBarObj;
        for (var j = 0, len2 = menuBarObj.childNodes.length; j < len2; j++) {
            if (menuBarObj.childNodes[j].enabled) {
                menuBarObj.childNodes[j].className = menuBarObj.childNodes[j].itemClassName;
                if (menuBarObj.childNodes[j].iconObj) {
                    menuBarObj.childNodes[j].iconObj.className = menuBarObj.childNodes[j].iconClassName;
                }
            }
        }
...
*/
function menuBarItemClick(pe) { // Private method
    if (this.enabled) {
        if (this.menu) {
            if (this.clicked) {
                this.className = this.itemClassNameOver;
...
function jsDOMenuBar() { // Public method
    ...
    this.menuBarObj.onclick = function(pe) { // Private method
        var e;
        if (!pe) {
            e = window.event;
            e.cancelBubble = true;
