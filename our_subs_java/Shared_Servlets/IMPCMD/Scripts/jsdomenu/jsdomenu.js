...
*/
function getX(pe) { // Private method
    var e;
    if (!pe) {
        e = window.event;
    }else{
...
*/
function getY(pe) { // Private method
    var e;
    if (!pe) {
        e = window.event;
    }else{
...
*/
function menuItemClick(pe) { // Private method
...
*/
function activatePopUpMenu(pe) { // Private method
    if (!popUpMenuObj) {
        return;
    }
...
*/
function leftClickHandler(pe) { // Private method
    if (getX(pe) > getClientWidth() || getY(pe) > getClientHeight()) {
        return;
    }
    var e;
    if (!pe) {
...
function addMenuItem(menuItemObj) { // Public method
    ...
    if (menuItemObj.displayText == "-") {
        ...
        itemElm.onclick = function(pe) { // Private method
            var e;
            if (!pe) {
                e = window.event;
                e.cancelBubble = true;
...
function hideAllMenus() { // Public method
    ...
    for (var j = 0, len = staticMenuId.length; j < len; j++) {
        refreshMenuItems(getElmId(staticMenuId[j]));
    }
}
...
function hideVisibleMenus() { // Public method
    ...
    if (typeof(staticMenuBarId) == "object") {
        for (var k = 0, len2 = staticMenuBarId.length; k < len2; k++) {
            refreshMenuBarItems(getElmId(staticMenuBarId[k]));
        }
    }
}
