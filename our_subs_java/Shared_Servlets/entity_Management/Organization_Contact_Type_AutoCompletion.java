/*
 * Organization_Contact_Type_AutoCompletion.java
 *
 * Created on June 23, 2007, 3:35 PM
 */
 ...
 * @version
 */
public class Organization_Contact_Type_AutoCompletion extends HttpServlet {
    /** Returns a short description of the servlet.
     */
    public String getServletInfo() {
        return "Organization_Contact_Type AutoCompletion Servlet";
    }
    ...
    throws ServletException, IOException {
        ...
        String organization_Type_Id = request.getParameter("organization_Type_Id");
        if(organization_Type_Id == null){
            web_Process.log("no organization_Type_Id passed on call.");
            do_Respond_Invalid(response);
        }
        ...
        if(action.equals("nameLookup")){
            ...
            Organization_Contact_Type temp_Organization_Contact_Type;
            try {
                temp_Organization_Contact_Type = new Organization_Contact_Type();
            ...
            try {
                temp_Organization_Contact_Type.setComp_Id(comp_Id);
                temp_Organization_Contact_Type.setOrganization_Type_Id(organization_Type_Id);
                String organization_Contact_Type_Id = request.getParameter(form_Field_Id_Of_Id);
                if (organization_Contact_Type_Id != null)
                    temp_Organization_Contact_Type.setOrganization_Contact_Type_Id(organization_Contact_Type_Id);
            }catch(DFException dfe){
                web_Process.log("Organization_Contact_Type_AutoCompletion  : " + dfe.getMessage());
                do_Respond_Invalid(response);
                return;
            }
            
            String where_Clause = " WHERE " +
            temp_Organization_Contact_Type.getComp_Id().toWhere_Clause() + " AND " +
            temp_Organization_Contact_Type.getOrganization_Type_Id().toWhere_Clause();
            if(!temp_Organization_Contact_Type.getOrganization_Contact_Type_Id().getValue().trim().equals(""))
                where_Clause = where_Clause + " AND " + temp_Organization_Contact_Type.getOrganization_Contact_Type_Id().toWhere_Range_Clause();
            
            String sort_Clause = " ORDER BY " +
            temp_Organization_Contact_Type.getOrganization_Contact_Type_Id().getField_Name().toString() + " ASC ";
            
            
            Organization_Contact_Type[] organization_Contact_Types;
            try {
                organization_Contact_Types = (Organization_Contact_Type[]) web_Process.getDatabase_Front().getMany_Records(temp_Organization_Contact_Type, where_Clause +
                sort_Clause, 0, 10);
            }
            ...
            if(organization_Contact_Types != null){
                StringBuffer sb = new StringBuffer();
                for(int i=0;i<organization_Contact_Types.length;i++){
                    sb.append("<organization_Contact_Type>");
                    sb.append("<id>" + StringPlus.XML_Encode(organization_Contact_Types[i].getOrganization_Contact_Type_Id().toString()) + "</id>");
                    sb.append("</organization_Contact_Type>");
                ...
                response.getWriter().write("<organization_Contact_Types>" + sb.toString() + "</organization_Contact_Types>");
    ...
    String form_Field_Id_Of_Id, String form_Field_Id_Of_Comp_Id, String form_Field_Id_Of_Organization_Type_Id,
    ...
    throws WPFatal_Exception {
        ...
        form_Field_Id_Of_Id, form_Field_Id_Of_Comp_Id, form_Field_Id_Of_Organization_Type_Id);
    }
    ...
    String form_Field_Id_Of_Id,  String form_Field_Id_Of_Comp_Id, String form_Field_Id_Of_Organization_Type_Id){
        ...
        // <input name-organization_Contact_Type_Title ... onKeyUp=form_Field_Id_Of_Id_DoCompletion()
        ...
        out.println("var " + form_Field_Id_Of_Id + "_Organization_Type_Id;");
        ...
        out.println("    " + form_Field_Id_Of_Id + "_Organization_Type_Id = document.getElementById(\""+form_Field_Id_Of_Organization_Type_Id+"\");");
        ...
        out.println("        var url = \"Organization_Contact_Type_AutoCompletion"+web_Process.get_HTML_Of_Href_Call_Parameters()+
        "&action=nameLookup"+
        "&comp_Id=\"+escape(" + form_Field_Id_Of_Id + "_Comp_Id.value)+\""+
        "&organization_Type_Id=\"+escape(" + form_Field_Id_Of_Id + "_Organization_Type_Id.value)+\""+
        ...
        out.println("");
        out.println("	var organization_Contact_Types = responseXML.getElementsByTagName(\"organization_Contact_Types\")[0];");
        out.println("	//alert(\"organization_Contact_Types.childNodes.length=\"+ organization_Contact_Types.childNodes.length);");
        out.println("    " + form_Field_Id_Of_Id + "_ClearTable();");
        out.println("    if (organization_Contact_Types.childNodes.length > 0) {");
        ...
        out.println("    }");
        out.println("");
        out.println("    //alert(\"organization_Contact_Types : \" + organization_Contact_Types);");
        out.println("");
        out.println("    for (loop = 0; loop < organization_Contact_Types.childNodes.length; loop++) {");
        out.println("	var organization_Contact_Type = organization_Contact_Types.childNodes[loop];");
        out.println("        //alert(\"loop : \" + loop + \"  organization_Contact_Type=\" + organization_Contact_Type.value);");
        out.println("        var id = organization_Contact_Type.getElementsByTagName(\"id\")[0];");
        ...
        out.println("        linkElement.setAttribute(\"value\", \"organization_Contact_Type\");");
