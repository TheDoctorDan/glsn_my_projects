/*
 * Security_Profile_Program.java
 *
 * Created on November 14, 2006, 9:26 AM
 */
 ...
 * @author  dgleeson
 */
public class Security_Profile_Program extends Database_Record_Base {
    ...
    // Empty Constructor
    /** Creates a new instance of Security_Profile_Program
     * @throws Database_Record_Constructor_Exception
     */
    public Security_Profile_Program() throws Database_Record_Constructor_Exception {
        super("Security Profile Program", "Security Profile Programs");
        try {
            this.security_Profile_Id =  Security_Profile.DFInit_Security_Profile_Id();
            this.program_Name =  Program.DFInit_Program_Name();
            this.access_Type =  new DFString_Enumerated("access_Type", "Access Type", Security_Profile.ACCESS_TYPES);


            DFBase[] index =   { this.security_Profile_Id, this.program_Name };
    ...
    /** Creates a new instance of Security_Profile_Program with key fields set to passed values.
     * @throws Database_Record_Constructor_Exception
     */
    public Security_Profile_Program(String security_Profile_Id, String program_Name) throws Database_Record_Constructor_Exception {
        this();
        try {
    ...
    public String get_Table_Name() {
        return "security_profile_program";
    }
     ...
     * @return Security_Profile_Program[] the size of array_Size as an Database_Record_Base[].
     */
    public Database_Record_Base[] get_Array_of_Records(int array_Size) {
        Security_Profile_Program[] security_Profile_Program_Records;
        security_Profile_Program_Records = new Security_Profile_Program[array_Size];
        return security_Profile_Program_Records;
    }
    ...
    public String getSQL_Create_Table() {
        ...
        out.append("        PRIMARY KEY  (`" + security_Profile_Id.getField_Name() + "`, `" + program_Name.getField_Name() + "`)\n");
    ...
    public String getNext_Sort_Clause(int which_Index) throws Database_Front_Exception {
        switch(which_Index){
            case PRIMARY_INDEX:
                return "ORDER BY security_Profile_Id ASC, program_Name ASC";
            default:
        ...
        case PRIMARY_INDEX:
            ...
            case 0:
                return "WHERE security_Profile_Id = '" + SQLUtil.encode(this.security_Profile_Id.toString()) + "' "
                    + " AND program_Name > '" + SQLUtil.encode(this.program_Name.toString()) + "' ";
            case 1:
                return "WHERE security_Profile_Id > '" + SQLUtil.encode(this.security_Profile_Id.toString()) + "' ";
            default:
                return null;
     ...
     * @return Security_Profile_Program record.
    ...
    public synchronized Database_Record_Base resultSet_Row_Decode(java.sql.ResultSet resultSet) throws Database_Record_Constructor_Exception {
        Security_Profile_Program result;
        try {
            result = new Security_Profile_Program();
     ...
     */
    public synchronized Security_Profile_Program copy_Of() throws Database_Record_Constructor_Exception {
        Security_Profile_Program result;
        try {
            result = new Security_Profile_Program();

            result.security_Profile_Id.setValue(this.security_Profile_Id.getValue());
            result.program_Name.setValue(this.program_Name.getValue());
    ...
    throws Database_Record_Constructor_Exception {
        ...
        this.program_Name.setRequest_Error("");
            
        Program program = null;

        try {
            program = Program.get_Program(this.program_Name.getValue(), database_Front);
        }catch(Database_Front_Exception dbfe){
            this.program_Name.setRequest_Error("Database Error. " + dbfe.getMessage());
            error_Count++;
        }catch(Database_Front_No_Results_Exception dbfe){
            this.program_Name.setRequest_Error("No Such Program.");
            error_Count++;
        }
