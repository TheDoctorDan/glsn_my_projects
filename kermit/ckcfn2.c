...
input() 
{
	...
	if (sstate != 0) {			/* If a start state is in effect, */
		...
		*data = '\0';
		...
		type = rpack(&len, &num, data); /* Else, try to read a packet. */

	/* If it's the same packet we just sent, it's an echo.  Read another. */

	if (type == sndtyp) 
		type = rpack(&len, &num, data);
...
int	num, len; 
{
	...
	tlco += spktl;
	if (pktlog) 
		zsoutl(ZPFILE, sndpkt);	/* If logging packets, log it */
...
{				/* Send the old packet again. */
	...
	for (w = 0; w < timint - 2; w++) {	/* Be extra sure no stuff is */
		...
		sleep(1);
		if (!ttchk() ) 
			ttinc(1);	/* be extra sure no stuff in SIII/V */
		if (!ttchk() ) 
			break;
	}
	if (*sndpkt)
	...
	if (pktlog && *sndpkt) 
		zsoutl(ZPFILE, sndpkt); /* Log packet if desired */
}
...
char	*dat; 
{
	...
	if (gotsoh) 
		...
		return('Q');	/* No SOH in 3 tries, fail. */
	...
	while (!done) {
		...
		if (t == 3) 
			cccount++;		/* Count any control-C's */

		if (t == eol) 
			return('Q');
		*l = kerm_unchar(t);			/* Packet length */
		...
		*n = kerm_unchar(t);
		...
		if ((type == 'S') || (type == 'I')) 
			pbl = 1;	/* Heuristics for  */
		else if (type == 'N') 
			pbl = *l - 2;    /* syncing block check type */
		else 
			pbl = bctu;
		...
		*l -= (pbl + 2);		/* Now compute data length */
		...
		if (j != pbl) 
			continue;		/* Block check right length? */
		done = 1;			/* Yes, done. */
	}
...
int	timo; 
{
	...
	if (c < 0) 
		return(c); 		/* Get a character */
	if (parity) 
		c = c & 0177;		/* If parity on, discard parity bit. */
...
inlin() 
{
	...
	maxt = (speed >= 110) ? (MAXTRY * 19200 / speed) : MAXTRY;
	...
	if (i < 1) 
		return(-1);		/* No characters, return. */
	if (pktlog) 
		zsoutl(ZPFILE, recpkt);	/* Log any we got, if logging. */
	if (k > maxt) 
