...
/*
 Variables available to outside world:
   ...
   flfnam -- Name of lock file, including its path, e.g.,
		"/var/spool/uucp/LCK..cul0" or "/etc/locks/tty77"
...
int	*lcl, modem; 
{
	...
	if (ttyfd > -1) 
		return(0);		/* If already open, ignore this call */
	...
	if (*lcl == -1) {
		if (strcmp(ttname, CTTNAM) == 0) {   /* "/dev/tty" always remote */
			...
			xlocal = (strncmp(ttname, cname, DEVNAMLEN) == 0) ? 0 : 1;
	...
	if(modem==0){
		fprintf(stderr,"Warning! not set to any modem. Direct connect assumed.  Open will wait for carrier\n");
	}
#ifdef UXIII
	/* if modem connection, don't wait for carrier */
	sprintf(dmys, "/usr/gleeson/bin/modechange %s", ttname);
	system(dmys);
/*	ttyfd = open(ttname, O_RDWR | (modem ? O_NDELAY : 0) ); */
	ttyfd = open(ttname, O_RDWR | O_NDELAY  ); 

	file_flags= fcntl(ttyfd, F_GETFL, 0);
	file_flags &= ~O_NDELAY;
	fcntl(ttyfd, F_SETFL, file_flags);
#else
...
#ifdef KERLD
	if (kerld) 
		ioctl(ttyfd, TIOCSETD, &oldld); /* Restore old line discipline. */
#endif
#ifdef UXIII
	if (ioctl(ttyfd, TCSETAW, &ttold) < 0) 
		return(-1); /* restore termio stuff */
#else
	if (stty(ttyfd, &ttold) < 0) 
		return(-1); /* Restore sgtty stuff */
#endif
#ifdef KERLD
	if (kerld) 
		ioctl(ttyfd, TIOCSETC, &oldc); /* Restore old special chars. */
#endif
...
#else
#ifdef ATT3BX
	char	*lockdir = "/var/spool/locks";
#else
	char	*lockdir = "/var/spool/uucp";
#endif
#endif
...
{		/* lock uucp if possible */
	...
	old_umask=umask(0);
	lck_fil = creat(flfnam, 0644);	/* create lock file ... */
	umask(old_umask);
	if (lck_fil < 0) 
...
int	speed, flow; 
{
	...
	if (kerld) {
		...
		if (ioctl(ttyfd, TIOCSETC, &newc) < 0) 
			return(-1);
	}
#endif
	...
	if (s > -1) 
		ttraw.c_cflag &= ~CBAUD, ttraw.c_cflag |= s; /* set speed */
...
int	speed, flow; 
{
	...
	if (s > -1) 
		tttvt.c_cflag &= ~CBAUD, tttvt.c_cflag |= s; /* set speed */
...
{			/* Set an interrupt trap. */
	...
	if (backgrd) 
		return;		/* must ignore signals in bkgrd */
...
*/
	...
	if (conif) 
		return;			/* Nothing to do if already on. */
...
{				/* Console-no-interrupts */
	...
	if (!isatty(0)) 
		return(0);		/* only for real ttys */
	if (backgrd) 
		return;		/* Ignore signals in background */
