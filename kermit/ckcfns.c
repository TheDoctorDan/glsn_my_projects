	...
	while ((a = *buf++) != '\0') {
		if (rptflg) {			/* Repeat processing? */
			if (a == rptq) {		/* Yes, got a repeat prefix? */
				rpt = kerm_unchar(*buf++);	/* Yes, get the repeat count, */
		...
		for (; rpt > 0; rpt--) {	/* Output the char RPT times */
			...
			if ((*fn)(a) < 0) 
				return(-1); /* Send it to the output function. */
		}
	}
	return(0);
}
...
{			/* Get next character */
	int	x; 
	CHAR a;			/* The character to return. */
...
reof() 
{
	int	x;
	if (cxseen == 0) 
		cxseen = (*data == 'D');	/* Got discard directive? */
...
char	data[]; 
{
	...
	x = (len-- > 0) ? kerm_unchar(data[0]) : DSPSIZ;	        /* Packet size */
	if (spsizf == 0)
		spsiz = (x < 10) ? DSPSIZ : x;

	x = (len-- > 0) ? kerm_unchar(data[1]) : DMYTIM;		/* Timeout */
	if (timef == 0)
		timint = (x < 0) ? DMYTIM : x;
	...
	seol = (len-- > 0) ? kerm_unchar(data[4]) : '\r';	/* Terminator  */
	if ((seol < 2) || (seol > 037)) 
		seol = '\r';
...
char	*vdir; 
{
	vdir[kerm_unchar(*vdir) + 1] = '\0';	/* End with a null */
	if (zchdir(vdir + 1)) {
		encstr(vdir + 1);
		ack1(data);
