...
#ifdef AMIGA
#define KERMRC "s:kermit-startup"
#else
#ifdef vax11c
...
unsigned char	c1;
	...
	int	pid, 			/* process id of child (modem reader) */
		parent_id; 		/* process id of parent (file reader) */
...
*/
	case XXSHE:                             /* Local shell command */
		 {
			...
			if ((pid = fork()) == 0) {          /* Make child */
				...
				exit(GOOD_EXIT);
			}              /* Just punt if it didnt work */ 
			    else {                              /* Parent */

				int	wstat;                      /* Kermit must wait for child */
				t_signal	(*istat)(), (*qstat)();
	...
	case XXUPL:                             /* upload */
		...
		if ((y = cmifi("ascii file name", "", &s, &x)) < 0) {
			if (y == -3) {
				printf("?A file specification is required\n");
				return(-2);
			} else
		...
		if (x != 0) {
			printf("?Wildcards not allowed in ascii file name\n");
			return(-2);
		}
		strcpy(line, s);                     /* Make a safe copy of the string */
		if ((y = cmcfm()) < 0)
			return(y);
		if ((upload_fp = fopen(line, "r")) == NULL) {
			perror(line);
			debug(F110, "Failure to open", line, 0);
		} else {
			...
			if (pid == -1) {
				perror("Can't create upload fork");
				printf("[Back at Local System]\n");
				return(0);
			}
			if (pid) {
				active=1;
				while ((c = getc(upload_fp)) != EOF && active) {
					if (ttoc(dopar(c)) <= -1) {
	...
	case XXOTP:                             /* other-protocol */
		...
		if (cmtxt("Unix shell command to execute for protocol", "", &s) < 0)
			return(-1);
		if(strlen(s)==0){
			fprintf(stderr,"need a command to execute\n");
			return(-1);
		}
		...
		/* set up bi-directional pipe to chose command */


		/* set up link to bc */
		if (pipe(p_to_child) < 0) {
			fprintf(stderr, "pipe failed\n");
			return(0);
		}
		if (pipe(p_from_child) < 0) {
			fprintf(stderr, "pipe failed\n");
			return(0);
		}
		...
		if ((child_pid = fork()) == 0) {
			...
			close(1);
			dup(p_from_child[1]);
fprintf(stderr,"execl for %s\n",line);
			ier=execl("/bin/sh", "sh", "-c", line, 0);
			fprintf(stderr,"child execl came back with ier=%d\n",ier);
			perror("child");
			exit(1);
		}
		if (child_pid < 0) {
		...
		close(p_from_child[1]);
			...
			pid = fork();			/* All ok, make a fork */
			if (pid == -1) {
				perror("Can't create other-protocol fork");
				printf("[Back at Local System]\n");
				return(0);
			}
			if (pid) {
				/* PARENT - read pipe, write tty port */
fprintf(stderr,"parent started\n");
				close(p_to_child[1]);
				active=1;
				while((ier = read(p_from_child[0], &c1, 1)) == 1 && active){
fprintf(stderr,"parent got a char:%d:%c:\n",c1,c1);
					if (ttoc(c1) <= -1) {
						perror("\r\nCan't send character");
						active = 0;
					}
...
fprintf(stderr,"Child started\n");
				/* CHILD  read tty port, write pipe */
				/* Inferior reads, prints port input */
				close(p_from_child[0]);
...
fprintf(stderr,"child got a char:%d:%c:\n",c,c);
c1=c;
					write(p_to_child[1], &c1, 1);
					while ((n = ttchk()) > 0) {	/* Any more left in buffer? */
						if (n > LBUFL) 
							n = LBUFL;   /* Get them all at once. */
						ttxin(n, lbuf);
...
shopar()
{
	...
	if (ebqflg)
		printf2("8th-Bit Prefix:      '%c'\n", ebq);
	if (rptflg)
		printf2("Repeat-Count Prefix: '%c'\n", rptq);

	printf("\nFile parameters:\n File Names:   ");
	if (fncnv)
		printf2("%-12s", "converted");
	else
		printf2("%-12s", "literal");
#ifdef DEBUG
