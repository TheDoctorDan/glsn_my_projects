...
void setdevice(struct device *ppage)
{
	...
	for (; ; ) {
		...
		if (!pgstate->cacheflag && !pgstate->nullflag) {
			if (page.depth != pgstate->dev.depth)
				pgstate->shadeok = 0;
			pgstate->dev = page;
		}
		if (i == 0)
...
struct point *centre, struct point *beg, struct point *end)
{
	...
	radm = gstate.ctm[0];
	if (gstate.ctm[1] > radm)
		radm = gstate.ctm[1];
	if (gstate.ctm[2] > radm)
		radm = gstate.ctm[2];
	if (gstate.ctm[3] > radm)
		radm = gstate.ctm[3];
	radm *= radaa[0];
...
void strokeinit(void)
{
	...
	if (gstate.dasharray.length != 0) {
		...
		for (; ; ) {
			...
			dashcnt++;
			dashsub++;
			if (dashsub == gstate.dasharray.length)
				dashsub = 0;
		}
	}
}
...
void clip(int rule)
{
	...
	for (count1 = gstate.clipbeg; count1 < gstate.pathend; count1++) {
		...
		if (ppoint->type != ptmove) {
			...
			clip1.flag = 0;
			clip1.swap = 0;
			clip1.x1 = floor(ppoint[-1].x * 256.0 + 0.5);
			clip1.y1 = floor(ppoint[-1].y * 256.0 + 0.5);
			clip1.x2 = floor(ppoint[0].x * 256.0 + 0.5);
			clip1.y2 = floor(ppoint[0].y * 256.0 + 0.5);
			clipline(&clip1);
		}
	}
