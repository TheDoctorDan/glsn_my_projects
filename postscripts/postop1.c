...
void ophandleerror(void)
{
	...
	errorflag = 2;
	token = errdstoken[0];
	if (token.type == typebool && token.value.ival)
		errormsg();
}
...
void opkram(void)
{
	...
	for (; ; ) {
		if (nest == 0)
			error(errunmatchedmark);
		if ((--token1)->type == typemark)
			break;
		nest--;
	}
...
void opaload(void)
{
	...
	if (opernest + length > operstacksize)
		error(errstackoverflow);
...
void opbegin(void)
{
	...
	if (dict->flags & flagrprot)
		error(errinvalidaccess);
	if (dictnest == dictstacksize)
		error(errdictstackoverflow);
...
void opcopy(void)
{
	...
	if (token2->type == typeint) {
		...
		if (num < 0 || num > opernest - 1)
			error(errrangecheck);
		if (opernest + num - 1 > operstacksize)
			error(errstackoverflow);
	...
	} else
	 {
		...
		if (token1->type == typestring) {
			...
			if (token1->length > token2->length)
				error(errrangecheck);
			memcpy(vmsptr(token2->value.vref), vmsptr(token1->value.vref), (int)token1->length);
...
void opeexec(void)
{
	...
	if (token1->type != typefile)
		error(errtypecheck);
	if (!equal(&execstack[execnest - 1], token1))
		error(errundefined);
	if (dictnest == dictstacksize)
		error(errdictstackoverflow);
...
void opexit(void)
{
	...
	while (nest >= istate.execbase) {
		...
		if (token1->flags & (flagrun | flagstop))
			break;
	...
	if (currtoken->flags & flagctrl) {
	...
	} else
	 {
		...
		if (token3->type != typeint) {
			type = typereal;
			if (token3->type != typereal)
				error(errtypecheck);
		}
		if (token4->type != typearray && token4->type != typepacked)
			error(errtypecheck);
		if (execnest + 6 > execstacksize)
			error(errexecstackoverflow);
		token = *token1;
		if (token.type != type) {
			token.type = typereal;
	...
	if (currtoken->flags & flagctrl) {
		...
		if (token1->type == typestring) {
			...
			for (; ; ) {
				...
				if (dict1->entries[length].key.type != 0)
					break;
				length++;
			}
