...
extern void rangecheck(int n, float *value, int minimum);
extern void callfunc(struct object *func, float *rp1, float *rp2, int minimum);
...
void rangecheck(int n, float *value, int minimum)
{
	float	v;
	while (n--) {
		v = *value;
...
void callfunc(struct object *func, float *rp1, float *rp2, int minimum)
{
...
void setupscreen(float *shade)
{
	...
	for (plane = 0; plane < gstate.dev.depth; plane++, hscreen++) {
		...
		htone->spotnext++;
		if (htone->spotnext > htone->spotmax)
			htone->spotnext = 0;
