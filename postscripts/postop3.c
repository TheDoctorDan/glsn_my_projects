...
void opsetcolorscreen(void)
{
	...
	memcpy((char *) gstate.screenfreq, (char*)freq, sizeof freq);
	memcpy((char *) gstate.screenangle, (char*)ang, sizeof ang);
...
void opsetdash(void)
{
	...
	if (token1->length != 0) {
		...
		if (dashtotal == 0.0)
			error(errrangecheck);
		memcpy((char *) gstate.dashlength, (char *) dashlength, (int)(len * sizeof (float)));
	}
