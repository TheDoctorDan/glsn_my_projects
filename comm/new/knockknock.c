...
extern void exit(), perror(), set_tty();
extern int	ioctl();
...
good_modem:
	...
	mktemp(filename);
	area.c_iflag = IGNBRK | ISTRIP | ICRNL | ERR_BEL ;
	area.c_oflag = OPOST | ONLCR | TAB3 ;
	area.c_cflag = B1200 | CS7 | PARENB | CREAD | HUPCL |CLOCAL ;
	area.c_lflag =0;
	area.c_line =0;
	area.c_cc[VINTR]= 255;
	area.c_cc[VQUIT]= 255;
	area.c_cc[VERASE]='\0010';
	area.c_cc[VKILL]= '\0030';
	area.c_cc[VMIN]= '\001';
	area.c_cc[VTIME]= '\001';
	area.c_cc[VSWTCH]= 255;
	call_area.attr= NULL;
	call_area.baud=1200;
	call_area.speed=1200;
	strcpy(dmy_tty,"/dev/tty00");
	call_area.line=NULL;
	strcpy(dmy_phone,"");
	call_area.telno=dmy_phone;
	call_area.modem=1200;
	fitty=dial(&call_area);
	if(fitty <0){
		fprintf(stderr,"dial failed with %d\n",fitty);
		perror("knockknock");
		return(fitty);
	}
