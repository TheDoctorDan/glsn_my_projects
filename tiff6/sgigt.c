#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/tools/RCS/sgigt.c,v 1.33 92/03/31 09:20:41 sam Exp $";
#endif
...
/*  fudge adjustment for window borders */
#define	YFUDGE	20
#define	XFUDGE	20
...
u_short	YCbCrHorizSampling, YCbCrVertSampling;
float	*YCbCrCoeffs;
float	*refBlackWhite;
int	isRGB = -1;
int	verbose = 0;
 ...
 */
static void
putcontig8bittile(cp, pp, Map, w, h, fromskew, toskew)
	register u_long *cp;
	register u_char *pp;
	register RGBvalue *Map;
	u_long w, h;
 ...
 */
static void
putcontig16bittile(cp, pp, Map, w, h, fromskew, toskew)
	register u_long *cp;
	register u_short *pp;
	register RGBvalue *Map;
	u_long w, h;
 ...
 */
static void
putseparate8bittile(cp, r, g, b, Map, w, h, fromskew, toskew)
	register u_long *cp;
 ...
 */
static void
putseparate16bittile(cp, r, g, b, Map, w, h, fromskew, toskew)
	register u_long *cp;
...
typedef void (*YCbCrPut)(u_long*, u_char*, int, int, u_long, int, int, int);
...
pickTileContigCase(RGBvalue* Map)
{
	...
	case PHOTOMETRIC_RGB:
		if (isRGB) {
			...
			put = (bitspersample == 8 ?
			    putcontig8bittile : putcontig16bittile);
		}
		break;
	case PHOTOMETRIC_PALETTE:
		switch (bitspersample) {
	...
	if (pick == 0) {
		TIFFError(filename, "Can not handle file");
		exit(-1);
	}
	return (put);
}
...
pickTileSeparateCase(RGBvalue* Map)
{
	...
	case PHOTOMETRIC_RGB:
		if (isRGB) {
			...
			put = (bitspersample == 8 ?
			    putseparate8bittile : putseparate16bittile);
		}
		break;
	}
	if (pick == 0) {
