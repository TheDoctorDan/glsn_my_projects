...
/*******************************************/
{
  ...
  if (spp == 1) {
      rv = loadPalette(tif, w, h, photo, bps);
  } else {
      rv = loadColor(tif, w, h, photo, bps, nc);
  }
  TIFFClose(tif);

  SetDirRButt(F_FORMAT, F_TIFF);
  return(rv);
}  
...
int   photo,bps;
{
  ...
  case PHOTOMETRIC_PALETTE:
	...
	SetISTR(ISTR_FORMAT, "TIFF, %u-bit, palette format", bps);
	break;
  case PHOTOMETRIC_MINISWHITE:
  case PHOTOMETRIC_MINISBLACK:
	SetDirRButt(F_COLORS, bps == 1 ? F_BWDITHER : F_GREYSCALE);
	SetISTR(ISTR_FORMAT,"TIFF, %u-bit, %s format.", 
	     bps,
	     photo == PHOTOMETRIC_MINISWHITE ?	"min-is-white" :
						"min-is-black");
	break;
  }
  sprintf(formatStr, "%lux%lu TIFF.",w,h);

  /* load up the XV global variables */
  pic = (byte *) malloc(w*h,1);
	...
	int stop;
{
	...
	/*  maybe should check photometric? */
	TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &width);
	TIFFGetField(tif, TIFFTAG_IMAGELENGTH, &height);
	/*  verify rwidth and rheight against width and height */
	stoponerr = stop;
	BWmap = NULL;
	PALmap = NULL;
